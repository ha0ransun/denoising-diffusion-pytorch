...
class GaussianProcessDiffusion(nn.Module):
    def __init__(
        self,
        model,
        *,
        ...
        super().__init__()
        assert not (type(self) == GaussianProcessDiffusion and model.channels != model.out_dim)
        assert not model.random_or_learned_sinusoidal_cond
    ...
    def p_sample(self, x, t: int, x_self_cond=None):
        ...
        if self.kernel is None:
            ...
            x_shape = x.shape
            noise = (self.kernel @ torch.randn_like(x).view(b, -1)).view(x_shape) if t > 0 else 0.  # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start
    ...
    def q_sample(self, x_start, t, noise=None):
        if self.kernel is None:
            ...
            noise = self.kernel @ default(noise, lambda: torch.randn_like(x_start))
