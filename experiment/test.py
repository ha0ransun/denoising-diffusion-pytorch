...
def get_all_entropy(x, num, batch_size):
    ...
    for i in tqdm(range(int(np.ceil(x.shape[0] / batch_size)))):
        ...
        cur_d = torch.round((torch.cdist(cur_x, x) ** 2) / 127.5 ** 2, decimals=6)
        for j in range(num):
            sigma = 0.002 + j * (80 - 0.002) / (num - 1)
...
def get_cov_entropy(x, num, batch_size):
    cov = torch.cov(x.T / 127.5) * 0.999 + 0.001 * torch.eye(x.shape[-1], device=x.device)
